...
<div class="game-section" id="gs-guide">
  <div class="scroll-body" style="padding:calc(var(--safe-top) + 12px) 18px 40px">
    ...
    ${section('🎭', '팀 및 특수 역할 배정', `
      ...
      ${roleRow('👑 엘리트', '번개 마일리지 2배 적립', '팀의 핵심 마일리지 기여자. 단, 투표로 적발되면 마일리지가 0.5배로 급감합니다.')}
    ...
    ${section('🗳️', '투표 및 정체 공개', `
      ...
      ${row('진행', '상대 팀으로 의심되는 1명을 지목(필수)하고, 확신이 있으면 그 사람의 역할까지 추가로 지목합니다(선택 · 애매하면 기권). 더블 역할은 2표를 행사합니다.')}
      ${row('팀 공개', '최다 득표자는 팀 소속이 타임라인에 공개되고, 마일리지가 영구적으로 50% 감소합니다.')}
      ${row('역할 공개', '그 사람을 지목한 인원의 60% 이상이 같은 역할로 지목했고 실제 역할과 일치하면, 역할이 공개되고 해당 특수 능력이 박탈됩니다. (예: 엘리트 2배 중단)')}
